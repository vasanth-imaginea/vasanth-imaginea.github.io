...
app.controller('MainCtrl', function($scope,$http) {
  $http.get('configure.json').success(function(response){ /* Fetching the JSON data */
    ...
    $scope.selectedOperation=response.operations[0] /* Selecting the first element by default */
  });
  $scope.submit=function(){
    ...
    for(var  i=0;i<selectedOperation.options.length;i++){
     ...
     $scope.result+=" "+($scope.searchText || "")/* Concatenating the values of checked fields and displaying */
  }
});